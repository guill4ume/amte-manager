...
<template>
	<GroupBox title="Inventory (Backpack)">
		<v-table density="compact" v-if="inventory.length">
			<thead>
				<tr>
					<th>Slot</th>
					<th></th>
			...
			<tbody>
				<tr v-for="item in inventory" :key="item.SlotPosition">
					<td class="text-grey">{{ item.SlotPosition - inventoryStart + 1 }}</td>
					<td>
		...
		<v-alert v-else type="info" density="compact">Backpack is empty</v-alert>
	</GroupBox>
</template>
